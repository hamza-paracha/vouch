...
export function doctor(env: NodeJS.ProcessEnv = process.env) {
  const issues: string[] = [];
  if (Number(process.versions.node.split(".")[0]) < 20) issues.push("Node 20 or newer is required");
  ...
  return { status: issues.length ? "not_ready" : "ready", version: VERSION, node: process.version, chromiumInstalled: existsSync(chromium.executablePath()),
    runtimeFingerprint: runtimeFingerprint(), modelCallsAllowed, issues, note: "Local checks only; no model calls and no credentials returned." };
}
