...
try {
  ...
  await writeFile(join(output, "package.json"), JSON.stringify({ name: "proof-jev-guard", version: source.version, private: true, type: "module",
    ...
    engines: source.engines, bin: { "proof-jev-guard": "bin/guard.mjs", "vouch-guard": "bin/guard.mjs", "vouch-jev-guard": "bin/guard.mjs" }, files: ["bin", "src", "README.md", "LICENSE"],
    dependencies: Object.fromEntries(["@modelcontextprotocol/sdk", "@typesafe-ai/sdk", "tsx", "zod"].map(name => [name, source.dependencies[name]])),
  }, null, 2) + "\n");
  // Seed exact resolved versions from proof-jev's tested lock rather than resolving a new dependency graph.
